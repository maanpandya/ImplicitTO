...
def main(nelx,nely,volfrac,penal,rmin,ft):
	...
	# FE: Build the index vectors for the coo matrix format (vectorized).
	KE=lk()
	...
	edofMat = np.stack([2*n1+2, 2*n1+3, 2*n2+2, 2*n2+3, 2*n2, 2*n2+1, 2*n1, 2*n1+1], axis=1)
	# Construct the index pointers for the coo format
	iK = np.kron(edofMat,np.ones((8,1))).flatten()
	jK = np.kron(edofMat,np.ones((1,8))).flatten()    

	# Filter: Build (and assemble) the index+data vectors (fully vectorized).
	ceil_rmin = int(np.ceil(rmin))
	dk = np.arange(-(ceil_rmin-1), ceil_rmin)
	dl = np.arange(-(ceil_rmin-1), ceil_rmin)
	DK, DL = np.meshgrid(dk, dl, indexing='ij')               # (ndk, ndl)
	fac_kernel = rmin - np.sqrt(DK**2.0 + DL**2.0)            # (ndk, ndl)

	IX, JY = np.meshgrid(np.arange(nelx), np.arange(nely), indexing='ij')  # (nelx, nely)
	# Broadcast to (nelx, nely, ndk, ndl)
	IX4 = IX[:, :, np.newaxis, np.newaxis]
	JY4 = JY[:, :, np.newaxis, np.newaxis]
	K_abs = IX4 + DK[np.newaxis, np.newaxis, :, :]
	L_abs = JY4 + DL[np.newaxis, np.newaxis, :, :]
	fac4  = np.broadcast_to(fac_kernel[np.newaxis, np.newaxis, :, :], K_abs.shape)

	mask = (fac4 > 0.0) & (K_abs >= 0) & (K_abs < nelx) & (L_abs >= 0) & (L_abs < nely)
	...
	while change>0.04 and loop<2000:
		loop=loop+1
		...
		# Plot to screen (throttled to every 5 iterations to reduce GUI overhead)
		if loop == 1 or loop % 5 == 0 or change <= 0.01:
